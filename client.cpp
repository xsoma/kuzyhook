...
void Client::DrawHUD() {
	...
	std::string text = tfm::format(XOR("kuzyhook v3 | Build date: " __DATE__ " | "  __TIME__ " | latency: %i ms"), fps, ms);
...
void Client::ClanTag()
{
	...
	auto SetClanTag = [&](std::string tag) -> void {
		...
		SetClanTagFn(tag.c_str(), XOR("kuzyhook"));
	};

	std::string szClanTag = XOR("kuzyhook");
	...
	if (g_menu.main.misc.autoaccept.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 3.6) % 10) {
			switch (int(g_csgo.m_globals->m_curtime * 3.6) % 1) {
			case 0: {SetClanTag("kuzyh00k"); break; }
			case 1: {SetClanTag("kuzyhook"); break; }
			default:;
			}
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::green, XOR("[kuzyhook] "));
