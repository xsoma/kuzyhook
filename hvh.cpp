...
void HVH::GetAntiAimDirection() {
	...
	case 0:
		...
		if (g_hvh.m_left)
			m_direction = m_view + 75.f;
		if (g_hvh.m_right)
			m_direction = m_view - 75.f;
		if (g_hvh.m_back)
			m_direction = m_view + 5.f;
		break;
	...
	if (m_yaw > 0) {
		...
		if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air) && !g_input.GetKeyState(g_menu.main.movement.fakewalk.get())) {
			...
			if (g_menu.main.antiaim.body_fake_stand_fakewalk.get()) {
				...
				else if (g_menu.main.antiaim.yaw_stand.get() == 5)
				{
					if (m_flicks == 0 || m_flicks % 2)
						g_cl.m_cmd->m_view_angles.y = -g_menu.main.antiaim.distortion_swap_amount.get();
					else
						g_cl.m_cmd->m_view_angles.y = g_menu.main.antiaim.distortion_swap_amount.get();
		...
		// run normal aa code.
			...
			case 5: {
				...
				if (m_flicks % 2)
				{
					direction = std::fmod((g_csgo.m_globals->m_curtime * (speed * 20.f)), m_direction - g_menu.main.antiaim.distortion_swap_amount.get());
				}
				else
				{
					direction = std::fmod(-(g_csgo.m_globals->m_curtime * (speed * 20.f)), m_direction - g_menu.main.antiaim.distortion_swap_amount.get());
